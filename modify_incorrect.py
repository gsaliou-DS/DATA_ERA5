...
def verifier_fichiers_nc(dossier):
    ...
    if fichiers_incorrects:
        ...
        tronquer_fichiers_nc(dossier, fichiers_incorrects)
...
def tronquer_fichiers_nc(dossier, fichiers_incorrects):
    for fichier, debut, fin in fichiers_incorrects:
        chemin_fichier = os.path.join(dossier, fichier)
        tronquer_fichier_nc(chemin_fichier, debut[:4])

def tronquer_fichier_nc(chemin_fichier, annee):
    try:
        ds = xr.open_dataset(chemin_fichier)
